# Contributing to ep_help_bubbles

Thanks for helping improve `ep_help_bubbles` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_help_bubbles` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_help_bubbles from your local clone
pnpm run plugins i --path ../ep_help_bubbles

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_help_bubbles/` are picked up after a server restart.
...
* Etherpad version, ep_help_bubbles version, browser + OS
* Steps to reproduce
